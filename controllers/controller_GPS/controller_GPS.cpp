...
struct OLuoi {
  ...
  int16_t so_lan_tham = 0; // số lần đi qua ô này (Heatmap)
};
...
// Gồm: header(1) + trạng thái(1) + vị trí(1) + yaw/ô lưới(1) + separator(1)
//     + %bản đồ(1) + số ô trạng thái(1) + bản đồ(HANG+2) + footer(1) = 8 +
//     LUOI_SO_HANG
static constexpr int DASHBOARD_DONG_TRANG_THAI =
    7; // dòng trước bản đồ (header+FSM+vitri+yaw+sep+banDo%+oluoi)
...
static void ve_dashboard(int trang_thai_fsm, double x, double y, double yaw,
  ...
  {
    ostringstream s;
    s << ANSI_BOLD << ANSI_CYAN << "══ ROBOT HUT BUI │ Buoc:" << setw(6)
      << g_buoc << " ══" << ANSI_RESET;
  ...
  {
    ostringstream s;
    s << "  Yaw    : " << ANSI_CYAN << fixed << setprecision(4) << setw(9)
  ...
  // ── Dòng 6-7: Thống kê bản đồ ───────────────────────────────
  {
    ...
    {
      ostringstream s;
      s << "  Ô lưới : " << ANSI_GREEN << ANSI_BOLD
        << "[1] Đã thăm : " << setw(4) << da_tham << ANSI_RESET << "   "
        << ANSI_WHITE << ANSI_BOLD << "[0] Chưa thăm: " << setw(4) << chua_tham
        << ANSI_RESET << "   " << ANSI_RED << ANSI_BOLD
        << "[2] Vật cản : " << setw(4) << vat_can << ANSI_RESET;
      dong_dashboard(s.str());
    }
  }
...
struct TrangThaiRobot {
  // Vị trí (lấy trực tiếp từ GPS)
  double vi_tri_x = 0.0;
  double vi_tri_y = 0.0;
  double quang_duong_tong = 0.0;
  int luu_gc_hien_tai = -1;
  int luu_gr_hien_tai = -1;

  // Để tính vận tốc góc từ IMU
  double goc_truoc = 0.0;
  bool buoc_dau = true; // true = chưa đọc góc lần đầu
...
int main(int argc, char **argv) {
  ...
  // Đọc vị trí ban đầu từ GPS (cần step 1 lần để cảm biến có dữ liệu)
  if (gps) {
    robot->step(buoc_ms);
    const double *vi_tri_gps = gps->getValues();
    if (vi_tri_gps) {
      rs.vi_tri_x = vi_tri_gps[0]; // Webots GPS: X thế giới
      rs.vi_tri_y = vi_tri_gps[1]; // Webots GPS: Y thế giới
  ...
  while (robot->step(buoc_ms) != -1) {
    ...
    // --- Vị trí từ GPS (liên tục, không tích lũy sai số) ---
    if (gps) {
      const double *vi_tri_gps = gps->getValues();
      if (vi_tri_gps) {
        double dx = vi_tri_gps[0] - rs.vi_tri_x;
        double dy = vi_tri_gps[1] - rs.vi_tri_y;
        rs.quang_duong_tong += sqrt(dx * dx + dy * dy);
    ...
    if ((unsigned)gc < (unsigned)LUOI_SO_COT &&
      ...
      // Tăng số lần thăm khi sang ô mới
      if (gc != rs.luu_gc_hien_tai || gr != rs.luu_gr_hien_tai) {
    ...
    case TIEN: {
      ...
      // Khi tiếp cận tường gần (<0.4m), kiểm tra góc xiên qua LiDAR ±10°.
      // Nếu lệch > 0.05m → dùng KP_QUAY mạnh hơn để căn thẳng kịp thời.
      {
        float d_lt = lidar_tai(anh_lidar, 350); // 10° lệch trái trước
        float d_pt = lidar_tai(anh_lidar, 10);  // 10° lệch phải trước
        bool gap_tuong_gan = (lidar_tai(anh_lidar, 0) < 0.4f);
        bool tiep_can_xien = gap_tuong_gan && (fabs(d_lt - d_pt) > 0.05f);
        co = tiep_can_xien
                 ? KP_QUAY * chuan_hoa_goc(rs.huong_hang - yaw) // tăng gain
                 : KP_GIU_THANG *
                       chuan_hoa_goc(rs.huong_hang - yaw); // bình thường
      }
    ...
    case QUAY_1:
      co = KP_QUAY * chuan_hoa_goc(rs.goc_muc_tieu - yaw);
      if (abs(chuan_hoa_goc(rs.goc_muc_tieu - yaw)) < NGUONG_SAI_SO_GOC) {
        // Snap goc_muc_tieu về bội số π/2 gần nhất tính từ huong_goc.
        // Dùng yaw thực tế (IMU) thay vì goc_muc_tieu để triệt tiêu
        // sai số tích lũy sau nhiều vòng quay.
        {
          double d = chuan_hoa_goc(yaw - rs.huong_goc);
          rs.goc_muc_tieu = chuan_hoa_goc(
              rs.huong_goc + round(d / (M_PI * 0.5)) * (M_PI * 0.5));
        }
    ...
    case QUAY_2:
      co = KP_QUAY * chuan_hoa_goc(rs.goc_muc_tieu - yaw);
      if (abs(chuan_hoa_goc(rs.goc_muc_tieu - yaw)) < NGUONG_SAI_SO_GOC) {
        // Snap huong_hang về bội số π/2 gần nhất tính từ huong_goc.
        // Dùng yaw thực tế (IMU) để loại bỏ sai số tích lũy.
        // Đây là fix chính tránh robot lệch góc dẫn đến kẹt tường.
        {
          double d = chuan_hoa_goc(yaw - rs.huong_goc);
          rs.huong_hang = chuan_hoa_goc(rs.huong_goc +
                                        round(d / (M_PI * 0.5)) * (M_PI * 0.5));
        }
